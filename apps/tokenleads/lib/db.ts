...
// Read-only handle to the GridLeads source database (myapp). Same cluster,
// same socket — useDb() just switches the namespace. NEVER write through this.
export async function sourceDb() {
  const m = await dbConnect();
  return m.connection.useDb('myapp', { useCache: true });
}
